if(view_current == view) {
    ...
    if(!is_undefined(amaze)) {
        if(!amaze.stack.isempty()) {
            ...
            msg = "Start = (" + string(amaze.start.xpos) + ", " + string(amaze.start.ypos) + ")" + " Seed = " + string(amaze.seed);
            draw_set_colour(c_red);
            draw_rectangle((amaze.start.xpos * 64) + 16, (amaze.start.ypos * 64) + 16, (amaze.start.xpos * 64) + 48, (amaze.start.ypos * 64) + 48, false);
        }
    }
